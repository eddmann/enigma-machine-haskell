RUN := docker run -v $(PWD):/src -v $(PWD)/.cabal:/root/.cabal:delegated -v $(PWD)/.ghc:/root/.ghc:delegated -w /src
IMAGE := haskell:8
EXECUTABLE := enigma
...
init:
	$(RUN) $(IMAGE) bash -c "cabal update && cabal install hspec QuickCheck --lib"
